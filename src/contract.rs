...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::CreateInsurance {
            id_insurance,
            margin,
        } => execute::create_insurance(deps, _env, info, id_insurance, margin),
        ExecuteMsg::AddModerator { new_moderator } => {
            let addr = deps.api.addr_validate(&new_moderator)?;
            execute::add_moderator(deps, _env, info, addr)
        }
        ExecuteMsg::DeleteModerator {
            moderator_to_remove,
        } => {
            let addr = deps.api.addr_validate(&moderator_to_remove)?;
            execute::delete_moderator(deps, _env, info, addr)
        }
        ...
        ExecuteMsg::UpdateInvalidInsurance {
            id_insurance
        } => execute::update_invalid_insurance(deps, info, id_insurance),
        ExecuteMsg::ClaimInsurance {
            id_insurance
        } => execute::claim_insurance(deps, info, id_insurance),
        ExecuteMsg::RefundInsurance {
            id_insurance,
        } => execute::refund_insurance(deps, info, id_insurance),
        ExecuteMsg::CancelInsurance {
            id_insurance
        } => execute::cancel_insurance(deps, info, id_insurance),
        ExecuteMsg::ExpireInsurance {
            id_insurance
        } => execute::expire_insurance(deps, _env, info, id_insurance),
        ExecuteMsg::LiquidateInsurance {
            id_insurance
        } => execute::liquidate_insurance(deps, _env, info, id_insurance),
    }
}
